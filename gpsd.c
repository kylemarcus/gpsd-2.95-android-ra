...
int send_to_server(int type, uint16_t port, struct timespec *tm)
{
    ...
    serv_addr.sin_port = htons(ACCOUNTING_PORT);
    ...
    printf("Message sent to server, closing the socket ...\n");
    return 0;
...
/* bind a passive command socket for the daemon */
{
	...
	port = ntohs((uint16_t) pse->s_port);
    else if ((port = (uint16_t) atoi(service)) == 0) {
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < AFCOUNT; i++) {
		...
		/* Resource Accounting */
		int port = ntohs(((struct sockaddr_in *)&fsin)->sin_port);	
